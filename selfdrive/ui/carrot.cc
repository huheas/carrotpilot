...
public:
    ...
    void drawHud(UIState* s) {
        ...
        // 只有在实际超速时才显示警告
        bool actually_speeding = (v_ego * 3.6 > xSpdLimit);
        NVGcolor stroke_color = COLOR_WHITE;
        ...
        if (true) {
            ...
            if (xSpdLimit > 0 && xSignType != 22) {
                ...
                // 只有在实际超速时才闪烁红色/黄色
                bool is_over_limit = (v_ego * 3.6 > disp_speed + 2);
                if (is_over_limit) {
                    limit_color = (blink_timer <= 8) ? COLOR_RED_ALPHA(210) : COLOR_YELLOW_ALPHA(210);
                } else {
                    limit_color = COLOR_WHITE_ALPHA(210);
                }
    ...
    void makeDeviceInfo(const UIState* s) {
        SubMaster& sm = *(s->sm);
        if (!sm.alive("deviceState") || !sm.valid("deviceState")) {
            // deviceState数据不可用时保持之前的值，不重置为0
            return;
        }
        ...
        try {
            ...
            if (cpuTempC.size() > 0) {
                ...
                cpuTemp = temp_sum / static_cast<float>(cpuTempC.size());
            }
        } catch (...) {
            // 保持之前的值
        }
        ...
        if (sm.alive("peripheralState") && sm.valid("peripheralState")) {
    ...
    void drawDeviceInfo(const UIState* s) {
        ...
        QString str = "";
        str.sprintf("GPU:%.0f%% MEM:%d%% DISK:%.0f%% CPU:%.0f%%,%.0f\u00B0C", gpuUsage, memoryUsage, freeSpace, cpuUsage, cpuTemp);
        NVGcolor top_right_color = (cpuTemp > 85.0 || memoryUsage > 85.0 || gpuUsage > 85.0) ? COLOR_ORANGE : COLOR_WHITE;
